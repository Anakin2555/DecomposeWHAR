...
class Feature_extractor_1DCNN_HAR_SSC(nn.Module):
    def __init__(self, input_channels, cnnhidden_dim,cnnout_dim, window_size, kernel_size = 3, stride = 1, dropout = 0):
        super(Feature_extractor_1DCNN_HAR_SSC, self).__init__()

        self.conv_block1 = nn.Sequential(
            nn.Conv1d(input_channels, cnnhidden_dim, kernel_size=1,
        ...
        self.conv_block2 = nn.Sequential(
            nn.Conv1d(window_size, cnnout_dim, kernel_size=5, stride=1, bias=False, padding=2),
        ...
        # self.conv_block2 = nn.Sequential(
        #     nn.Conv1d(num_hidden, embedding_dimension, kernel_size=kernel_size, stride=1, bias=False, padding=2),
...
def Mask_Matrix(num_node, time_length, decay_rate):
    ...
    for i in range(time_length):
        v = 0
        for r_i in range(i,time_length): # 遍历上三角
